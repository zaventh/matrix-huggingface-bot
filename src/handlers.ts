...
import {MessageEvent, RelatesTo, StoredConversation, StoredConversationConfig} from "./interfaces.js";
...
export default class CommandHandler {
    ...
    constructor(private client: MatrixClient, private chatGPT: HfInferenceEndpoint) {
    }
    ...
    private async onMessage(roomId: string, event: MessageEvent) {
        try {
            ...
            try {
                result = await sendChatGPTMessage(this.chatGPT, await bodyWithoutPrefix, storedConversation)
            } catch (error) {
            ...
            await Promise.all([
                ...
                sendReply(this.client, roomId, this.getRootEventId(event), `${result.generated_text}`, MATRIX_THREADS, MATRIX_RICH_TEXT)
            ]);
